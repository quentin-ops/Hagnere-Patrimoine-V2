...
export function WorldMap({
  ...
  const svgMap = map.getSVG({
    radius: 0.22,
    color: theme === "dark" ? "#FFFFFF30" : "#00000060",
    shape: "circle",
    backgroundColor: "transparent",
  });

  const projectPoint = (lat: number, lng: number) => {
    // Correct Mercator projection for world map
    const mapWidth = 800;
    const mapHeight = 400;

    // Convert longitude to x coordinate (longitude range: -180 to 180)
    const x = ((lng + 180) / 360) * mapWidth;

    // Convert latitude to y coordinate with Mercator correction
    // Latitude range: -90 to 90, but we'll use a simpler linear projection
    ...
    <div className="w-full aspect-[2/1] bg-transparent rounded-lg relative font-sans">
      ...
      >
        ...
        <defs>
          ...
          {/* Gradient for the animated pulse */}
